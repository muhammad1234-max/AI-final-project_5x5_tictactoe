...
class BoardSizeScreen:
    def __init__(self, root, callback):
        self.root = root
        self.callback = callback  # Function to call with selected board size
        ...
        # Title
        self.title_label = tk.Label(self.main_frame, text="SELECT BOARD SIZE", 
                                  font=self.title_font, bg=self.bg_color, fg=self.accent_color)
        self.title_label.pack(pady=(20, 10))
        
        # Subtitle
        self.subtitle_label = tk.Label(self.main_frame, text="Choose the size of the game board", 
        ...
        # Create size selection buttons with hover effects
    ...
    def create_size_buttons(self):
        """Create stylish board size selection buttons"""
        sizes = [
            {"name": "3×3", "color": self.size_colors['3'], "hover": "#66bb6a", "value": 3,
             "description": "Classic tic-tac-toe size"},
            {"name": "5×5", "color": self.size_colors['5'], "hover": "#ffa726", "value": 5,
             "description": "Medium size board"}
        ]
        
        self.size_buttons = []
        
        # Create a row of 2 buttons (instead of a 2x2 grid)
        for i, size in enumerate(sizes):
            # Create button frame for each size with padding
            button_frame = tk.Frame(self.buttons_frame, bg=self.bg_color, padx=15, pady=15)
            button_frame.grid(row=0, column=i, padx=20, pady=10)
            
            # Button with rounded corners effect
            button = tk.Button(button_frame, text=size["name"], font=self.button_font,
                             bg=size["color"], fg=self.text_color, width=8, height=2,
                             relief=tk.FLAT, cursor="hand2",
                             command=lambda s=size: self.select_size(s))
            button.pack(pady=(0, 5))
            
            # Description label
            desc_label = tk.Label(button_frame, text=size["description"],
                                font=("Helvetica", 10), bg=self.bg_color, fg=self.text_color,
                                wraplength=150)
            desc_label.pack()
            
            # Store button reference and add hover effects
            self.size_buttons.append((button, size))
            
            # Add hover effects
            button.bind("<Enter>", lambda event, b=button, h=size["hover"]: self.on_button_hover(b, h))
            button.bind("<Leave>", lambda event, b=button, c=size["color"]: self.on_button_leave(b, c))
    ...
    def animate_entrance(self):
        """Animate the entrance of the board size selection screen"""
    ...
    def select_size(self, size):
        """Handle board size selection with animation"""
        # Flash effect on selected button
        button = next(b for b, s in self.size_buttons if s["name"] == size["name"])
    ...
    def fade_out(self, selected_size, alpha=1.0):
        ...
        if alpha > 0:
            ...
            self.root.after(30, lambda: self.fade_out(selected_size, alpha))
        else:
            # Destroy board size selection screen widgets and call the callback with selected size
            for widget in self.main_frame.winfo_children():
                widget.destroy()
            self.main_frame.destroy()
            
            # Call callback with size info
            self.callback(selected_size["value"])
