    ...
    def check_win(self, player):
        ...
        for row in range(board_size):
            for col in range(board_size - 4):
                if all(board_state[row][col+i] == player for i in range(5)):
                    return True
        
        # Check vertical wins
        for row in range(board_size - 4):
            for col in range(board_size):
                if all(board_state[row+i][col] == player for i in range(5)):
                    return True
        
        # Check diagonal wins (top-left to bottom-right)
        for row in range(board_size - 4):
            for col in range(board_size - 4):
                if all(board_state[row+i][col+i] == player for i in range(5)):
                    return True
